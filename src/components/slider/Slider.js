...
const Slider = () => {
   ...
   const renderSlider = (sliderItems, count) => {
      const renderedSliderItems = sliderItems
         .map(({ ...props }) => (
      ...
      for (let i = 1; i <= count; i++) {
         renderedDots.push(
            <div onClick={() => setSlide(i)} key={uuidv4()} className={`slider__dot ${i === current ? 'active' : ''}`}></div>
         )
      }
   ...
   const { renderedSliderItems, renderedDots } = renderSlider(sliderItems, count);
