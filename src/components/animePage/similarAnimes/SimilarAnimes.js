...
const SimilarAnimes = () => {
	...
	const similarAnimeLoadingStatus = useSelector((state) => state.anime.similarAnimeLoadingStatus);
	...
	const renderSimilarAnime = (arr) => {
		return arr.map(({ title, id }) => <SimilarAnimeItem key={uuidv4()} title={title} id={id} />);
	};
