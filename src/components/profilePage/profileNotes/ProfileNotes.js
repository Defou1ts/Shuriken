...
const ProfileNotes = () => {
	...
	const renderNotes = (notesTypes) => {
		return notesTypes.map(({ name, text }) => {
			...
			const count = notes.filter((note) => note.note === name).length;
			return (
				<li
					onClick={() => dispatch(setActiveNotesFilter(name))}
					key={uuidv4()}
					className={`${className} ${activeNotesFilter === name ? 'active' : ''}`}
				>
					{text}
	...
	if (!notes) {
		return <p className="profile__notes-not-found">У вас пока ничего нет в закладках</p>;
	}
	...
	renderedNotes.unshift(
		<li
			...
			key={uuidv4()}
			className={`profile__note ${activeNotesFilter === 'all' ? 'active' : ''}`}
		>
			Все
			<div className="profile__note-count">{Object.values(notes).length}</div>
		</li>
	);
